...
6. Add an association to the model you want to have comments: has_many :comments, :class_name => "BigComment::Comment", :as => :commentable

7. Add these two lines where you want the comment threads to show up:
%= render "big_comment/comments/list", :commentable => commentable %
%= render "big_comment/comments/new", :commentable => commentable %

8. Define commentable.to_summary, which is used in the subject line of notification emails.
